  ...
  return (
    <div className="w-full bg-font-color-light rounded-lg p-6 flex flex-col gap-6">
      <figure className="w-full bg-white rounded-md aspect-square relative overflow-hidden">
        <p
          className={`p-2 bg-quinary text-white rounded-md absolute top-4 left-4 ${
      ...
      <div className="flex-grow flex flex-col gap-2">
        <h2 className="font-bold text-2xl text-quinary">{data.newsName}</h2>
        <p>{data.description}</p>
      </div>
      <div>
        <p className="text-quinary font-medium text-lg">Fecha</p>
        <p>{new Date(data.newsDate).toISOString().split("T")[0]}</p>
      </div>
      <input
        type="button"
        value="Ir a la actuacion urbanistica asociada"
        className="bg-quinary text-white rounded-md text-lg font-semibold w-full py-2 cursor-pointer hover:bg-senary transition-all"
        onClick={() =>
